...
(function ($) {
    ...
    EditableForm.prototype = {
        ...
        **/
        []: function() {
            ...
            /**
            Fired when []ing starts
            @event []ing
            @param {Object} event event object
            **/
            this.$div.triggerHandler('[]ing');
            ...
            .then($.proxy(function () {
                ...
                /**
                Fired when form is []ed
                @event []ed
                @param {Object} event event object
                **/
                this.$div.triggerHandler('[]ed');

                this.showForm();

                //call post[] method to perform actions required visibility of form
                if(this.input.post[]) {
                    this.input.post[]();
                }
            }, this));
    ...
    //to display form you should call '[]' method
    $form.editableform('[]');
    */
    ...
    $.fn.editableform.defaults = {
        ...
        **/
        ...
        Otherwise newValue simply []ed into element.
    ...
    Popup.prototype = {
                ...
                $(document).on('click.editable', function(e) {
                    var $target = $(e.target), i,
                        exclude_classes = ['.editable-container',
                                           ...
                                           '.datepicker', //in inline mode datepicker is []ed into body
                                           '.modal-backdrop',
                    ...
                    for(i=0; i<exclude_classes.length; i += 1) {
        ...
        */
        ...
        []Form: function() {
            ...
            .on({
                ...
                []ing: $.proxy(this.setPosition, this), //this allows to place container correctly when loading shown
                resize: $.proxy(this.setPosition, this), //this allows to re-position container when form size is changed
                []ed: $.proxy(function(){
                    /**
                    Fired when container is shown and form is []ed (for select will wait for loading dropdown options).
        ...
        **/
        ...
        show: function (closeAll) {
            ...
            /*
            Currently, form is re-[]ed on every show.
...
(function ($) {
    ...
    Editable.prototype = {
        ...
        init: function () {
            ...
            //if display is function it's far more convinient to have autotext = always to [] correctly on init
            ...
            //depending on autotext run []() or just finilize init
            $.when(doAutotext ? this.[]() : true).then($.proxy(function() {
                if(this.options.disabled) {
                    this.disable();
                } else {
                    this.enable();
...
(function ($) {
    ...
    AbstractInput.prototype = {
       ...
       /*
       this method called before [] to init $tpl that is inserted in DOM
       */
       pre[]: function() {
           this.$tpl = $(this.options.tpl); //whole tpl as jquery object
           this.$input = this.$tpl;         //control itself, can be changed in [] method
           this.$clear = null;              //clear button
           this.error = null;               //error message, if input cannot be []ed
       },

       /**
        []s input from tpl. Can return jQuery deferred object.
...
(function ($) {
    ...
    $.extend(List.prototype, {
        ...
        onSourceReady: function (success, error) {
            ...
            //if (typeof source === 'object' && Promise.resolve(source) == source)
            //{
            //    pp = source.then(function (r) { return r }).then(function (r) { console.log(r); return r });
            //    console.log(pp);
            //}
            //loading from url
    ...
    List.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
        ...
        Since 1.4.1 key `children` supported to [] OPTGROUP (for **select** input only):
        @example
        [
...
(function ($) {
    ...
    $.extend(NumberInput.prototype, {
         []: function () {
            NumberInput.superclass.[].call(this);
            this.setAttr('min');
            this.setAttr('max');
            this.setAttr('step');
        },
...
(function ($) {
    ...
    var Constructor = function (options) {
        ...
        //store function that []s text in select2
...
(function ($) {
    ...
    Constructor.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
        ...
        **/
        ...
        If <code>false</code> clear button will not be []ed.
...
(function ($) {
    ...
    $.fn.editableform.buttons =
      '<button type="submit" class="btn btn-primary btn-sm editable-submit">'+
        '<i class="fad fa-check" aria-hidden="true"></i>'+
      '</button>'+
...
(function( $ ) {
	...
	var DPGlobal = {
		...
		parseDate: function(date, format, language) {
			...
			date = new UTCDate(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0);
			...
			if (parts.length == fparts.length) {
				...
				for (var i=0, s; i<setters_order.length; i += 1){
					s = setters_order[i];
		...
		formatDate: function(date, format, language){
			...
			for (var i=0, cnt = format.parts.length; i <= cnt; i += 1) {
				if (seps.length)
                                {
		...
		headTemplate: '<thead>'+
							'<tr>'+
								'<th class="prev"><i class="fad fa-arrow-left"/></th>'+
								'<th colspan="5" class="datepicker-switch"></th>'+
								'<th class="next"><i class="fad fa-arrow-right"/></th>'+
							'</tr>'+
						'</thead>',
...
(function ($) {
    ...
    DateField.defaults = $.extend({}, $.fn.editabletypes.date.defaults, {
        ...
        @property tpl
        **/
        tpl:'<div class="input-append date"><input type="text"/><span class="add-on"><i class="fad fa-th"></i></span></div>',
        /**
        @property inputclass
        @default 'input-small'
        **/
...
(function ($) {
    ...
    var Constructor = function (options) {
    console.log('ta constructor', options);
        ...
        this.options.sourceCache = false;
    ...
    $.extend(Constructor.prototype, {
        fetchData: function(query, sync, async) {
            var src = tasource;
            if ($.isFunction(src)) {
                src = src();
            }
            $.get(src + '?limit=10&search=' + query + '%').done(function(data){ async(data); });
        },
        []List: function() {
            this.$input = this.$tpl.is('input') ? this.$tpl : this.$tpl.find('input[type="text"]');
            tasource = this.options.source;
            this.options.typeahead.display = function (x) {
                return typeof x == 'string' ? x : x.text;
            };
            this.options.typeahead.source = this.fetchData;
            //apply typeahead
            //var ta = this.$input.data('typeahead');
            //this.options.typeahead.[] = $.proxy(this.typeahead[], ta);
            //this.options.typeahead.select = $.proxy(this.typeaheadSelect, ta);
            //this.options.typeahead.move = $.proxy(this.typeaheadMove, ta);
            this.$input.typeahead(this.options.typeahead);

            //patch some methods in typeahead

            //this.$input.on('typeahead:[]',  this.typeahead[]);
            //this.$element.on('typeahead:[]',  function(e) { alert('element []'); });
            //this.$input.on('typeahead:select', this.typeaheadSelect);
            //this.$input.on('typeahead:cursorChange', this.typeaheadMove);
            //ta.[] = $.proxy(this.typeahead[], ta);
        ...
        value2input: function(value) {
            if(this.getIsObjects()) {
                var items = $.fn.editableutils.itemsByValue(value, this.sourceData, function(o) { return o.text; });
        ...
        */
        ...
        []Clear: $.fn.editabletypes.text.prototype.[]Clear,
        post[]: $.fn.editabletypes.text.prototype.post[],
